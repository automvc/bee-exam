...
//		condition.set("detailInfo", "test detailInfo");
		...
		orders.setDetailInfo("test detailInfo");
		
//		int updateNum=suidRich.update(orders, Orders_F.num,condition); //指定只更新num字段
		int updateNum = suidRich.updateBy(orders, condition, Orders_F.name,Orders_F.detailInfo); // 指定用name,detailInfo作为过滤条件
