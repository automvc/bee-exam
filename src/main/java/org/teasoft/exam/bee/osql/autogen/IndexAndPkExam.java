...
public class IndexAndPkExam {
	...
	public static void test() {
		...
		Ddl.indexNormal(new TestUser(), "username,lastName", "test_index_name2");//有字段转换
		//CREATE INDEX test_index2 ON test_user(username,last_name)
		
		//索引名称采用默认的
		Ddl.indexNormal(new TestUser(), "name");
		Ddl.indexNormal(new TestUser(), "username,lastName");//有字段转换
		
		
		//2.唯一索引
		Ddl.unique(new TestUser(), "name", "test_uk_index_name21");
		Ddl.unique(new TestUser(), "username,lastName", "test_uk_index_name22");//有字段转换
		
		//索引名称采用默认的
		Ddl.unique(new TestUser(), "name");
		Ddl.unique(new TestUser(), "username,lastName");//有字段转换
