...
import org.teasoft.exam.bee.osql.ds.InitDsUtil;
...
//			HoneyConfig.getHoneyConfig().multiDs_matchEntityClassPath = "ds2:org.teasoft.exam.bee.osql.entity.dynamic.Orders,org.teasoft.exam.bee.osql.moretable.entity.**;ds3:com.xxx.dd.User";
			...
			ExtInterceptor extIntercept = new ExtInterceptor();
//			extIntercept.setDataSourceOneTime("ds2"); //ok
			InterceptorChain interceptorChain=new DefaultInterceptorChain();
			interceptorChain.addInterceptor(extIntercept); //模拟测试
			BeeFactory.getHoneyFactory().setInterceptorChain(interceptorChain);
			//spring方式??
...
//            HoneyContext.setConfigRefresh(true);  //要显式刷新才会更改配置.
//            test3();
