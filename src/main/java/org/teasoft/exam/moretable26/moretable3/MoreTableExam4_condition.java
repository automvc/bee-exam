 ...
 * @since 3.0.0
 */
public class MoreTableExam4_condition {
...
//			List<Orders2026> list1 = moreTable.select(orders1, 2, 10); // select 查询前10条记录
//
//			if (list1 != null) {
...
//			}
			...
			condition
					// .op("myuser.id", Op.gt,6)
					// .op("user.id", Op.gt,6)
					.op("orders2026.id", Op.ge, 1002)
					.start(0).size(2) // 分页
			// .start(2).size(6) //分页
			;
//			
